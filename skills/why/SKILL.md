...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (open files, recent edits, the active file or selection, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, list the available MCP servers. Scan the tool list for `mcp__<server>__*` tools already in context, then run `ToolSearch` (query `mcp`, high `max_results`) to surface deferred MCP tools that are installed but not yet loaded. Both together are the roster. A server with no tools in either list is not installed; record that as a coverage gap, not a null result.
...
- `model`: `sonnet`. Investigation is bulk retrieval and quoting, not judgment.
- Read-only posture. Claude Code's Agent tool has no `readonly` flag, so state the restriction in the brief: read, run read-only commands, write nothing. That is a posture, not a sandbox. Do not use the `Explore` agent type here: it is scoped to file search and will not reach MCP tools.
...
- `subagent_type`: `general-purpose`
- `model`: `fable`. Synthesis is prose and confidence calibration.
- Read-only posture. Claude Code's Agent tool has no `readonly` flag, so state the restriction in the brief: read, run read-only commands, write nothing. That is a posture, not a sandbox. The synthesizer spot-verifies citations, so it needs the same MCP access the investigators had.
