---
description: Create comprehensive feature implementation plan with codebase analysis and pattern extraction (agent-aware)
argument-hint: <feature description | path/to/prd.md>
---

> Adapted from PRPs-agentic-eng by Wirasm. Part of the PRP workflow series.
> Agent-aware fork: adds Concern → Path Map (Phase 2) and agent-tagged tasks (Phase 6).

# PRP Plan (Agent-Aware)
...
Map this repo's layout to the five domain agent concerns. Inspect the directory structure,
build configs, and existing code to determine which paths belong to each concern. Handle
monolith, monorepo, or microservices — for multi-service repos, identify the specific
service(s) involved in this feature.

Record the mapping in this table (included verbatim in the plan output):

| concern  | paths (globs)                                  |
|----------|------------------------------------------------|
| database | [resolved from repo]                            |
| backend  | [resolved from repo]                            |
| frontend | [resolved from repo]                            |
| test     | [resolved from repo]                            |
| docs     | [resolved from repo]                            |

This map feeds the `scope:` field on every task in Phase 6.
...
```yaml
id: T1
agent: database | backend | frontend | test | docs
scope:
  - "[globs from Concern → Path Map]"
depends_on: []
validation: "[command]"
parallelizable: false
```
**Goal:** [What to do and why]

**Mirror:** [Pattern from Patterns to Mirror section to follow, with file:lines]
...
[Continue for all tasks in dependency order: database → backend → frontend → test → docs.
Keep one agent's tasks contiguous where dependencies allow.]

---

## Agent Assignments

| task | agent    | scope                        | depends_on | validation              |
|------|----------|------------------------------|------------|-------------------------|
| T1   | [agent]  | [scope globs]                | —          | [command]               |
| T2   | [agent]  | [scope globs]                | T1         | [command]               |
| ...  | ...      | ...                          | ...        | ...                     |
...
- **Agent Distribution**: [e.g. "2 backend, 1 database, 1 test, 1 docs"]

> Next step: Run `/prp-implement-team .claude/PRPs/plans/{name}.plan.md` to execute this plan.
```
...
- [ ] Every task has id, agent, scope, depends_on, validation, Goal, Mirror, and Acceptance
- [ ] No task requires additional codebase searching
- [ ] Import paths are specified
- [ ] GOTCHAs documented where applicable
- [ ] Every task's scope ⊆ its agent's Concern → Path Map row
...
### Agent Coverage
- [ ] Concern → Path Map populated for all five concerns
- [ ] Agent Assignments table matches task blocks
- [ ] Tasks ordered by dependency (database → backend → frontend → test → docs)
- [ ] Every task has a non-empty scope
